# Post-Mortem Report: SLLM Non-Determinism in Stage 1
...
This report documents the architectural failure observed during the deployment of the **Small Language Model (SLLM)** (1B-3B parameter class) for the highly frequent and highly constrained task of generating structured Conventional Commit messages (Pipeline **Stage 1**).

> See [Production Git Workflow Standards](./production_git_workflow_standards.md)

The underlying architectural decision to use the SLLM was based on achieving efficiency at the edge. However, subsequent analysis revealed a critical mismatch between the **task requirement (100% deterministic, low-context output)** and the **model's capability (non-deterministic synthesis)**.

| Metric | Detail | Status |
| :--- | :--- | :--- |
| **Issue Title** | Failure of 1B-3B SLLM to reliably generate structured Conventional Commit messages. | **CLOSED** |
...
| **Root Cause** | **Low Instruction Fidelity and Synthesis Limitation** in small (1B-3B) SLLMs. The model could not maintain complex, simultaneous constraints (parsing raw diff, applying multiple rules, generating structured JSON) while conserving VRAM. | **Architectural** |
| **Mandated Solution** | **Replace the LLM** in Stage 1 with a **Deterministic CLI Tool** (`gitlint`) to enforce the format. Limit the SLLM role to the final, high-context Stage 3 transformation. | **MANDATORY** |

> See [Tools: Commit & Changelog Tooling for Release Pipelines](./tools_commit_and_changelog_tooling_for_release_pipelines.md)

## 2. Introduction: Failure of a High-Frequency SLLM Application
...
* **Result:** The SLLM-generated commit messages failed to pass mandatory CI/CD gates, leading to a direct violation of the **[Production Git Workflow: Standards](./production_git_workflow_standards.md)**.
* **Action:** We are immediately retiring the SLLM from **Stage 1** of the pipeline. The function will be replaced by a deterministic, non-LLM CLI tool to enforce compliance via an interactive workflow.
...
The failure was a direct result of mismatching the task complexity with the model's resource constraints. We attempted to use the SLLM for a task that requires **high deterministic parsing capability** and **strict adherence to multiple rules**, a domain better suited to traditional software logic.

**The Two Points of Failure**

| Failure Type | Description | Observed SLLM Error |
| :--- | :--- | :--- |
...
1.  **Eliminate the LLM Role in Stage 1:** The 1B-3B SLLM is explicitly **retired** from the commit message generation task.
...
3.  **LLM Role Restriction:** The SLLM's function is now strictly limited to **Stage 3: Release Notes Transformation**, where it processes short, pre-validated `CHANGELOG.md` excerpts.

This mandate shifts the compliance burden from the brittle SLLM synthesis step to the **developer's interactive workflow**, guaranteeing a clean input for the subsequent automated stages.
...
1.  **SLLM Decommissioning (Stage 1 Retirement):** Remove all local configurations and system prompts related to the 1B-3B SLLM assisting with commit message generation. This role is permanently retired.
...
4.  **Training Material:** Training will be immediately deployed to outline the new manual yet validated commit workflow and the updated role of the SLLM (now exclusively **Stage 3: Transformation**).

## Appendix A. Prompts Examples

**SLLM Execution Sequence (Workflow)** - Expected

The developer initiates this sequence within the CLI agent (`aider`) to the local SLLM.
...
1.  **SLLM Process:** The 1B-3B model reads the instructions and performs a two-step transformation:
...
### 3. Ultra-Detailed System Prompt for 3B SLLM
