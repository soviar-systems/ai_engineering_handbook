...
- [x] **Step 1: Write test for basic token calculation and update**
...
- [x] **Step 2: Write test for updating missing `token_size`**
...
- [x] **Step 3: Write test for preserving file content**
...
- [x] **Step 1: Write comprehensive module docstring (Code-as-Doc)**
...
- [x] **Step 2: Implement token counting logic using `tiktoken`**
...
- [x] **Step 3: Implement frontmatter update logic with internal contracts**
...
- [x] **Step 4: Implement CLI and Discovery**
...
### Task 5: CI/CD and Hook Integration

**Files:**
- Modify: `.pre-commit-config.yaml`
- Modify: `.github/workflows/quality.yml`

- [ ] **Step 1: Verify Pre-commit Integration**
  - Ensure `check-frontmatter` hook is correctly configured to trigger the new accuracy validation on staged files.
  - Verify that `update_token_counts.py` is NOT a pre-commit hook (it's a utility for the developer, not a gate), but that `check-frontmatter` acts as the gate.

- [ ] **Step 2: Add Test Hook for Update Script**
  - Add `test-update-token-counts` hook to `.pre-commit-config.yaml` to run `pytest tools/tests/test_update_token_counts.py` whenever the script or its tests are modified.

- [ ] **Step 3: Update GitHub Actions**
  - Ensure the `quality.yml` workflow executes the frontmatter validation and the new update-script tests as part of the CI pipeline to block PRs.

- [ ] **Step 4: End-to-End "Adversary" Verification**
  - Intentionally modify a `token_size` value to be incorrect in a governed file.
  - Verify that `uv run tools/scripts/check_frontmatter.py` detects the error.
  - Verify that the pre-commit hook blocks the commit.
  - Verify that the CI pipeline (via GH Action simulation or actual push) would block the merge.
